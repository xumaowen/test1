...
tem=reshape(tem,6,128,128,64);%将读入的DTI数据排列成9*128*128*64的矩阵形式，存储在img中
fclose(fid);
tem_mri=cell(128,128,64);%构造128*128*64的空矩阵cell,赋值给mri用于存储张量体素的9个分量，其排列顺序为
...
%dtidata存放DTI数据
tem_dtidata=cell(128,128,64);%把mri中每个元素的9个分量按照DTI3*3对称矩阵的形式存储
tem_re_dtidata=cell(128,128,64);
for x=1:128
    for y=1:128
        for z=1:64
