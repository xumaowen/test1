...
dtitk_img=reshape(dtitk_img,6,256,256,256);%将读入的DTI数据排列成6*128*128*64的矩阵形式，存储在img中
fclose(fid);
dtitk_mri=cell(256,256,256);%构造128*128*64的空矩阵cell,赋值给mri用于存储张量体素的9个分量，其排列顺序为
...
%dtidata存放DTI数据
dtitk_dtidata=cell(256,256,256);%把mri中每个元素的9个分量按照DTI3*3对称矩阵的形式存储
re_dtitk_dtidata=cell(256,256,256);
for x=1:256
    for y=1:256
        for z=1:256
